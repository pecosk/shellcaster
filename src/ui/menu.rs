...
impl<T: Clone + Menuable> Menu<T> {
    ...
    pub fn update_items(&mut self) {
        ...
        let (map, _order_unused, order) = self.items.borrow();
        if !order.is_empty() {
...
impl Menu<Podcast> {
    ...
    pub fn get_episodes(&self) -> LockVec<Episode> {
        ...
        let (borrowed_map, _order_unused, borrowed_order) = self.items.borrow();
...
impl Menu<NewEpisode> {
    ...
    fn change_item_selections(&mut self, indexes: Vec<usize>, selection: Option<bool>) -> bool {
        ...
        {
            let (mut borrowed_map, borrowed_order, _filtered_unused) = self.items.borrow();
            for idx in indexes {
